...
static const char *autoswitch_itemsrus[] = {
	"НИКОГДА",
	"ВСЕГДА",
	"НОВОЕ",
	"ЛУЧШЕ",
...
{
	{"+scores",		"показать счёт",		ID_SHOWSCORES,	ANIM_IDLE,		K_TAB,			-1,		-1, -1},
	{"+button2",		"использовать предмет",		ID_USEITEM,		ANIM_IDLE,		K_ENTER,		-1,		-1, -1},
	{"+speed", 		"бег / ходьба",		ID_SPEED,		ANIM_RUN,		K_SHIFT,		-1,		-1,	-1},
	{"+forward", 		"идти вперёд",		ID_FORWARD,		ANIM_WALK,		K_UPARROW,		-1,		-1, -1},
	...
	{"+left", 		"повернуться влево",		ID_LEFT,		ANIM_TURNLEFT,	K_LEFTARROW,	-1,		-1, -1},
	{"+right", 		"повернуться вправо",		ID_RIGHT,		ANIM_TURNRIGHT,	K_RIGHTARROW,	-1,		-1, -1},
	{"+strafe", 		"красться",	ID_STRAFE,		ANIM_IDLE,		K_ALT,			-1,		-1, -1},
	{"+lookup", 		"повернуться вверх",		ID_LOOKUP,		ANIM_LOOKUP,	K_PGDN,			-1,		-1, -1},
	{"+lookdown", 		"повернуться вниз",		ID_LOOKDOWN,	ANIM_LOOKDOWN,	K_DEL,			-1,		-1, -1},
	{"+mlook", 		"управление мышкой",		ID_MOUSELOOK,	ANIM_IDLE,		'/',			-1,		-1, -1},
	{"centerview", 		"выравнять вид",		ID_CENTERVIEW,	ANIM_IDLE,		K_END,			-1,		-1, -1},
	{"+zoom", 		"прицел",		ID_ZOOMVIEW,	ANIM_IDLE,	-1,	-1,	-1, -1},
	{"weapon 1",		"пила",		ID_WEAPON1,	ANIM_WEAPON1,	'1',	-1,	-1, -1},
	{"weapon 2",		"автомат",		ID_WEAPON2,	ANIM_WEAPON2,	'2',	-1,	-1, -1},
	{"weapon 3",		"дробовик",		ID_WEAPON3,	ANIM_WEAPON3,	'3',	-1,	-1, -1},
	{"weapon 4",		"гранатомёт",	ID_WEAPON4,	ANIM_WEAPON4,	'4',	-1,	-1, -1},
	...
	{"weapon 8",		"плазмамёт",		ID_WEAPON8,	ANIM_WEAPON8,	'8',	-1,	-1, -1},
	{"weapon 9",		"БФГ",			ID_WEAPON9,	ANIM_WEAPON9,	'9',	-1,	-1, -1},
        {"weapon 10",		"крюк",		ID_WEAPON10,	ANIM_WEAPON10,	-1,	-1,	-1, -1},
        {"weapon 11",		"гвоздомёт",		ID_WEAPON11,	ANIM_WEAPON11,	-1,	-1,	-1, -1},
        {"weapon 12",		"мины",	ID_WEAPON12,	ANIM_WEAPON12,	-1,	-1,	-1, -1},
        {"weapon 13",		"пулемёт",		ID_WEAPON13,	ANIM_WEAPON13,	-1,	-1,	-1, -1},
	...
	{"+button3", 		"жест",			ID_GESTURE,		ANIM_GESTURE,	K_MOUSE3,		-1,		-1, -1},
	{"messagemode", 	"чат",				ID_CHAT,		ANIM_CHAT,		't',			-1,		-1, -1},
	{"messagemode2", 	"чат - командный",		ID_CHAT2,		ANIM_CHAT,		-1,				-1,		-1, -1},
	{"messagemode3", 	"чат - цель",	ID_CHAT3,		ANIM_CHAT,		-1,				-1,		-1, -1},
	{"messagemode4", 	"чат - враг",	ID_CHAT4,		ANIM_CHAT,		-1,				-1,		-1, -1},
        {"+voiprecord", 	"звуковой chat",           ID_VOIP_TALK,		ANIM_CHAT,		'q',				-1,		-1, -1},
	{(char*)NULL,		(char*)NULL,		0,				0,				-1,				-1,		-1,	-1},
};
...
if(rus.integer){
	b1 = g_bindingsrus[a->generic.id].bind1;
}
	if (b1 == -1)
		strcpy(name,"???");
	else
...
if(rus.integer){
		b2 = g_bindingsrus[a->generic.id].bind2;
}
		if (b2 != -1)
		{
...
if(rus.integer){
			UI_DrawString( x - SMALLCHAR_WIDTH, y, g_bindingsrus[a->generic.id].label, UI_RIGHT|UI_SMALLFONT, controls_binding_color );
}
...
if(!rus.integer){
	...
	s_controls.invertmouse.generic.flags |= QMF_GRAYED;
}
	...
	if(onandroid.integer == 1){
	s_controls.smoothmouse.generic.flags |= QMF_GRAYED;
	}
	...
	if(onandroid.integer == 1){
	s_controls.sensitivity.generic.flags |= QMF_GRAYED;
}
	...
	if(onandroid.integer == 1){
	s_controls.voip_talk.generic.flags |= QMF_GRAYED;
}
...
if(rus.integer){
	...
	s_controls.looking.generic.id	    = ID_LOOKING;
	s_controls.looking.generic.callback	= Controls_MenuEvent;
	s_controls.looking.generic.x	    = 32;
	s_controls.looking.generic.y	    = 240 - 2 * PROP_HEIGHT;
...
//	s_controls.looking.style			= UI_RIGHT;
	...
	s_controls.movement.generic.id	     = ID_MOVEMENT;
	s_controls.movement.generic.callback = Controls_MenuEvent;
	s_controls.movement.generic.x	     = 32;
	s_controls.movement.generic.y	     = 240 - PROP_HEIGHT;
...
//	s_controls.movement.style			= UI_RIGHT;
	...
	s_controls.weapons.generic.id	    = ID_WEAPONS;
	s_controls.weapons.generic.callback	= Controls_MenuEvent;
	s_controls.weapons.generic.x	    = 32;
	s_controls.weapons.generic.y	    = 240;
...
//	s_controls.misc.style			= UI_RIGHT;
	...
	s_controls.freelook.generic.flags		= QMF_SMALLFONT;
	s_controls.freelook.generic.x			= SCREEN_WIDTH/2;
	s_controls.freelook.generic.name		= "вертикальный обзор";
	s_controls.freelook.generic.id			= ID_FREELOOK;
	s_controls.freelook.generic.callback	= Controls_MenuEvent;
	s_controls.freelook.generic.statusbar	= Controls_StatusBar;
...
if(onandroid.integer == 1){
	...
	s_controls.alwaysrun.generic.flags	   = QMF_SMALLFONT;
	s_controls.alwaysrun.generic.x	       = SCREEN_WIDTH/2;
	s_controls.alwaysrun.generic.name	   = "всегда бежать";
	s_controls.alwaysrun.generic.id        = ID_ALWAYSRUN;
	s_controls.alwaysrun.generic.callback  = Controls_MenuEvent;
	s_controls.alwaysrun.generic.statusbar = Controls_StatusBar;
	...
	s_controls.autoswitch.generic.flags	    = QMF_SMALLFONT;
	s_controls.autoswitch.generic.x	        = SCREEN_WIDTH/2;
	s_controls.autoswitch.generic.name	    = "авто-переключение оружия";
	s_controls.autoswitch.generic.id        = ID_AUTOSWITCH;
	s_controls.autoswitch.generic.callback  = Controls_MenuEvent;
	s_controls.autoswitch.generic.statusbar = Controls_StatusBar;
        s_controls.autoswitch.itemnames = autoswitch_itemsrus;
...
if(onandroid.integer == 1){
	...
	s_controls.voip_teamonly.generic.flags	    = QMF_SMALLFONT;
	s_controls.voip_teamonly.generic.x	        = SCREEN_WIDTH/2;
	s_controls.voip_teamonly.generic.name	    = "командный звуковой чат";
	s_controls.voip_teamonly.generic.id        = ID_VOIP_TEAMONLY;
	s_controls.voip_teamonly.generic.callback  = Controls_MenuEvent;
	s_controls.voip_teamonly.generic.statusbar = Controls_StatusBar;
	...
	s_controls.joyenable.generic.flags	   = QMF_SMALLFONT;
	s_controls.joyenable.generic.x	       = SCREEN_WIDTH/2;
	s_controls.joyenable.generic.name	   = "джойстик";
	s_controls.joyenable.generic.id        = ID_JOYENABLE;
	s_controls.joyenable.generic.callback  = Controls_MenuEvent;
	s_controls.joyenable.generic.statusbar = Controls_StatusBar;
	...
	s_controls.joythreshold.generic.x		  = SCREEN_WIDTH/2;
	s_controls.joythreshold.generic.flags	  = QMF_SMALLFONT;
	s_controls.joythreshold.generic.name	  = "порог джойстика";
	s_controls.joythreshold.generic.id 	      = ID_JOYTHRESHOLD;
	s_controls.joythreshold.generic.callback  = Controls_MenuEvent;
	s_controls.joythreshold.minvalue		  = 0.05f;
	s_controls.joythreshold.maxvalue		  = 0.75f;
