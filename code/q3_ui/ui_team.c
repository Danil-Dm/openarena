...
//		UI_ForceMenuOff();
	...
	case ID_TAUNT:
		...
		break;
	case ID_TAUNTK:
		trap_Cmd_ExecuteText( EXEC_APPEND, "vstr tauntk\n" );
		UI_ForceMenuOff();
		break;
	case ID_KILL:
...
//		UI_ForceMenuOff();
		break;
	case ID_MUSICNEXT:
		trap_Cmd_ExecuteText( EXEC_APPEND, "MusicplayerNext\n" );
//		UI_ForceMenuOff();
		break;
	case ID_MUSICPREV:
		trap_Cmd_ExecuteText( EXEC_APPEND, "MusicplayerPrev\n" );
//		UI_ForceMenuOff();
		break;
	case ID_SOUND:
		UI_SoundOptionsMenu();
...
if(!rus.integer){
	...
	s_actmain.tauntk.string           = "Taunt 2";
...
if(rus.integer){
	...
	s_actmain.tauntk.generic.x        = 300;
	s_actmain.tauntk.generic.y        = y;
	s_actmain.tauntk.string           = "Жест 2";
	s_actmain.tauntk.style            = UI_CENTER|UI_SMALLFONT;
	s_actmain.tauntk.color            = colorRed;
	y += 25;
...
//	y += 50;
	...
	if(!rus.integer){
	s_actmain.building.generic.name		= "Building 0-no 1-yes ^12-R ^43-B:";
	}
	if(rus.integer){
	s_actmain.building.generic.name		= "Строительство 0-нет 1-да ^12-R ^43-B:";
	}
	s_actmain.building.generic.x			= 320;
	s_actmain.building.generic.y			= y;
	s_actmain.building.minvalue			= 0;
    s_actmain.building.maxvalue			= 3;
	...
	if(!rus.integer){
	s_actmain.oasbid.generic.name		= "Block Number:";
	}
	if(rus.integer){
	s_actmain.oasbid.generic.name		= "Номер Блока:";
	}
	s_actmain.oasbid.generic.x			= 320;
	s_actmain.oasbid.generic.y			= y;
	s_actmain.oasbid.minvalue			= 1;
    s_actmain.oasbid.maxvalue			= 20;
...
static void LoadConfig_MenuEvent( void *ptr, int event ) {
	...
	case ID_GO:
		...
		trap_Cvar_Set("MusicplayerMusic", "1");
...
static void MusicPlayerNext( void ) {
	...
	num = MusicplayerMusic.integer + 1;
	
if(num < 1){
	num = s_configs.list.numitems;
}
if(num > s_configs.list.numitems){
	num = 1;
}

		trap_Cvar_SetValue( "MusicplayerMusic", num);
		i = MusicplayerMusic.integer;
		trap_Cmd_ExecuteText( EXEC_APPEND, va( "music UserTracks/%s\n", s_configs.list.itemnames[i] ) );
		trap_Cmd_ExecuteText( EXEC_APPEND, va( "echo %s\n", s_configs.list.itemnames[i] ) );
...
static void LoadConfig_MenuInit( void ) {
	...
	s_configs.list.numitems			= trap_FS_GetFileList( "UserTracks", "wav", s_configs.names, NAMEBUFSIZE );
