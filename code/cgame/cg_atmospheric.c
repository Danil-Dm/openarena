 ...
 *
 * desc:		Add atmospheric effects to view.
 *              Current supported effects are rain and snow.
 *
 * $Archive: /source/code/cgame/cg_atmospheric.c 
 * $Author: unknown
 * $Revision: none 
 * $Modtime: 10/09/05 13:07p 
 * $Date: 10/09/05 13:07p 
 *****************************************************************************/
...
void CG_AddAtmosphericEffects()
{
  	...
  	max = 1000 ? (cg_atmFx.numDrops >> 1) : cg_atmFx.numDrops;
...
qboolean CG_AtmosphericKludge()
{
  	...
  	if( !Q_stricmp( cgs.mapname, "maps/am_lavaarena.bsp" ) )
  	{
